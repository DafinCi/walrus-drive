  ...
  return useQuery<WorkspaceInvite[]>({
    ...
    queryFn: async () => {
      ...
      if (!response.ok || !result.success) {
        throw new Error(
          result.error || "Unable to load the list of invitations.",
        );
      }
...
export function useCreateInvite() {
  ...
  return useMutation({
    mutationFn: async (payload: CreateInviteInput) => {
      ...
      if (!response.ok || !result.success) {
        throw new Error(result.error || "Unable to create the invitation.");
      }
...
export function useRevokeInvite() {
  ...
  return useMutation({
    mutationFn: async (payload: RevokeInviteInput) => {
      ...
      if (!response.ok || !result.success) {
        throw new Error(
          result.error || "Unable to revoke access for this invitation link.",
        );
      }
