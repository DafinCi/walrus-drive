...
export function LiveProofPlayground() {
  ...
  return (
    <div
      ...
      onDrop={handleDrop}
      className="w-full max-w-md bg-zinc-900/50 backdrop-blur-xs border border-zinc-800 rounded-2xl p-5 shadow-2xl relative overflow-hidden group hover:border-zinc-700 transition-colors"
    >
      ...
      {status === "idle" && (
        <div
          ...
          className="border border-dashed border-zinc-800 hover:border-sky-500/50 rounded-xl p-8 flex flex-col items-center justify-center text-center cursor-pointer transition-all bg-zinc-950/40 group/drop"
        >
          <input
            type="file"
            ref={fileInputRef}
        ...
        <div className="space-y-4 animate-in fade-in zoom-in-95 duration-200">
          <div className="p-3 bg-zinc-950/80 border border-zinc-800 rounded-xl space-y-2">
            ...
            <div className="flex items-start gap-1.5 pt-2 border-t border-zinc-900 font-mono text-[9px]">
              ...
              <span className="text-zinc-400 break-all bg-zinc-900/50 p-1.5 rounded-md border border-zinc-800/40 w-full">
                {fileHash}
              </span>
            </div>
          </div>
